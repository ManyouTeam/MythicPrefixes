...
public class LanguageManager {
    ...
    protected String getPlayerLanguage(Player player) {
        ...
        try {
            if (ConfigManager.configManager.getBoolean("config-files.per-player-language") && !MythicPrefixes.freeVersion) {
    ...
    private void initLanguages() {
        File langFolder = new File(MythicPrefixes.instance.getDataFolder(), "languages");
    ...
    private String getMessage(Player player, String key, String... args) {
        ...
        text = text.replace("{plugin_folder}", String.valueOf(MythicPrefixes.instance.getDataFolder()));
        return text;
    }

    private void saveLanguageFile(String lang, YamlConfiguration config) {
        File file = new File(MythicPrefixes.instance.getDataFolder(), "languages/" + lang + ".yml");
        try {
            config.save(file);
        } catch (IOException e) {
            e.printStackTrace();
